...
for cc in C:
    ...
    for train_index, test_index in kf:
        ...
        for cl in xrange(4):
            ...
            with open('./trainacc/c%s_r%d_cl%d.train' %(cc, rr, cl+1), 'w') as fp:
            ...
            model = subprocess.call('./svm_light/svm_learn -c %f %s %s' %(cc, './trainacc/c%s_r%d_cl%d.train' %(cc, rr, cl+1), './trainacc/c%s_r%d_m%d.model' %(cc, rr, cl+1)), shell=True)

        #for each model, each corresponding to one class, generate a prediction
            classified = subprocess.call('./svm_light/svm_classify %s %s %s' %('./trainacc/c%s_r%d_cl%d.train' %(cc, rr, cl+1), './trainacc/c%s_r%d_m%d.model' %(cc, rr, cl+1), './trainacc/c%s_r%d_p%d.trainpredict' %(cc, rr, cl+1)), shell = True)
        ...
        with open('./trainacc/c%s_r%d_p%d.trainpredict' %(cc, rr, 1)) as fp:
            predictions_1 = [float(line.rstrip('\n')) for line in fp]
            
        with open('./trainacc/c%s_r%d_p%d.trainpredict' %(cc, rr, 2)) as fp:
            predictions_2 = [float(line.rstrip('\n')) for line in fp]
        
        with open('./trainacc/c%s_r%d_p%d.trainpredict' %(cc, rr, 3)) as fp:
            predictions_3 = [float(line.rstrip('\n')) for line in fp]
        
        with open('./trainacc/c%s_r%d_p%d.trainpredict' %(cc, rr, 4)) as fp:
...
with open('./trainacc/accuracies.txt', 'w') as fp:
    ...
    #        for cl in xrange(4):
            ...
            #with open('c%s_r%d_t%d.test' %(cc, rr, cl+1), 'w') as fp:
